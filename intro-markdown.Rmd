...
```curve(equation, from= <lower x limit>, to= <upper x limit>, n= <number of evenly spaced points in domain>, 
xlab= <x axis labels>, ylab= <y axis labels>, col= <color of graph>```
...
So, how do we tell which dimensions we should analyze along? We want to pick out the dimensions that capture the most data, and PCA, or principal component analysis, is one method that can help us do that. 

> Tip: you can think of "component" and "dimension" as synonyms. So, any time you see "principle component" or "principle component analysis" you can read those as "principle dimension" and "principle dimension analysis".

Principle components represent the dimensions where there's the most variance, and the data is more spread out. A higher variance indicates that the dimension captures more of the data, so if you only analyze the components with the highest variances, you can still capture a lot of the data's underlying pattern. 
But why don't we just analyze all the components? That way we can capture all of the data, right?? Let's take a look at the graph below.

```{r, echo=FALSE}
curve(log10(5*x+1), from= 0, to= 15, xlab= "PCs analyzed", ylab= "% Data represented", xaxt="n", yaxt="n")
```

This logarithmic curve is an approximate representation of the relationship between PCs analyzed and % Data represented. You can see that after a certain point, the slope flattens out and even if you analyze more PCs, you're not really increasing the % Data represented by much. Analyzing more PCs takes more computing power, and after a certain point it might not be worth it to use all that power for only a small increase in % Data analyzed. This idea is what we call diminishing returns- the benefits gained (% data analyzed) aren't high enough to justify investing all that computing power.

TL;DR: PCA is one way to perform dimensionality reduction, which simplifies data by identifying which dimensions most accurately represent it. 
...
Now that we've run PCA on our data, we'll use some different methods to help us interpret and visualize the results.

### Interpretation and visualization

#### Eigenvalues

We can compare the amount of variance each dimension, or principal component, has using a value called the eigenvalue. Eigenvalues are high for the first PCs and lower for the next PCs, so the first PCs represent dimensions with the largest amount of variance and they're the best at capturing the data. 
...
We can also use something called a scree plot to graphically determine the number of PCs to analyze. Scree plots display the dimensions from smallest to largest eigenvalue. Usually, the scree plot is a steep curve that quickly flattens out, and the tallest bars on the far left represent the components with the highest eigenvalues.
...
Now that we know we can capture 33.8% of the data along dimensions 1 and 2, let's graph it. Here, the data is color-coded by target value. Each dot represents one person. A target value of 0 indicates no heart disease while a value of 1 indicates that that person has some kind of heart disease. 
...
Beckerman, Andrew P., Childs, Dylan Z., Petchey, Owen L. (2017) Getting Started with R: An Introduction for Biologists. Oxford, England: Oxford University Press.

Jolliffe IT, Cadima J. Principal component analysis: a review and recent developments. 2016;374(2065).
...
UCI machine learning repository: Heart disease data set. Uci.edu. https://archive.ics.uci.edu/ml/datasets/Heart+Disease